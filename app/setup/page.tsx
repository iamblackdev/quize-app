...
export default function SetupPage() {
  ...
  const loadingMessages = [
    'Consulting the medical textbooks… 📚',
    'Crafting your questions… ✍️',
    'Double-checking the anatomy… 🦴',
    'Almost ready… 🩺',
  ]
  ...
  const handleSubmit = async () => {
    ...
    try {
      const [filesPayload, pastQuestionsPayload] = await Promise.all([
        filesToPayload(uploadedFiles),
        filesToPayload(pastQuestionsFiles),
      ])
      ...
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          files: filesPayload,
          pastQuestionsFiles: pastQuestionsPayload,
        }),
      })
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-cream-50 border-b border-amber-light/30">
        <div className="max-w-2xl mx-auto px-6 py-5 flex items-center gap-4">
          <Link href="/" className="btn-ghost py-1.5 px-3 text-sm">← Back</Link>
          <div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="animate-fade-slide-up stagger-2 space-y-4">
          <div>
            <label className="label">Course Content / Notes <span className="font-normal text-charcoal-light">(optional)</span></label>
            <textarea
          ...
          <div>
            <label className="label">Upload PDFs or Images <span className="font-normal text-charcoal-light">(optional, multiple allowed)</span></label>
            <div
              onClick={() => courseFileRef.current?.click()}
              className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-200
                ${uploadedFiles.length > 0
                  ? 'border-rose-warm/60 bg-rose-warm/5'
                  : 'border-amber-light/50 hover:border-rose-warm/40 hover:bg-rose-warm/3'}`}
            >
              <input
                ref={courseFileRef}
                type="file"
              ...
              <p className="font-body text-sm text-charcoal-mid">
                {uploadedFiles.length > 0 ? 'Add more files' : 'Drop PDFs or images here, or click to browse'}
              </p>
        ...
        <div className="animate-fade-slide-up stagger-2 space-y-4">
          <div>
            ...
            <p className="text-xs text-charcoal-light font-body -mt-1 mb-2">
              The AI will reuse 2–3 of these in your quiz and match the style of the rest.
            </p>
            ...
            <div
              ...
              className={`relative border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all duration-200
                ${pastQuestionsFiles.length > 0
                  ? 'border-amber-warm/60 bg-amber-warm/5'
                  : 'border-amber-light/50 hover:border-amber-warm/40 hover:bg-amber-warm/5'}`}
            >
              <input
                ref={pastFileRef}
                type="file"
              ...
              <p className="font-body text-sm text-charcoal-mid">
                {pastQuestionsFiles.length > 0 ? 'Add more past papers' : 'Upload past exam papers (PDFs or images)'}
              </p>
            </div>
        ...
        <div className="animate-fade-slide-up stagger-3">
          <label className="label">Number of Questions: <span className="text-rose-warm font-bold">{questionCount}</span></label>
          <div className="flex items-center gap-4">
            ...
            <input
              ...
              className="flex-1 accent-rose-warm"
            />
        ...
        <div className="animate-fade-slide-up stagger-4">
          <label className="label">Question Types <span className="font-normal text-charcoal-light">(select all that apply)</span></label>
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-all duration-200
                    ${selected
                      ? 'border-rose-warm bg-rose-warm/8 shadow-sm'
                      : 'border-amber-light/30 bg-white/50 hover:border-amber-warm/40'}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-lg">{t.icon}</span>
                    <span className={`font-display font-bold text-sm ${selected ? 'text-rose-warm' : 'text-charcoal-warm'}`}>
                      {t.label}
                    </span>
                  </div>
        ...
        {/* Error */}
        {error && (
          <div className="bg-rose-50 border border-rose-warm/30 text-rose-warm rounded-xl p-4 text-sm font-body">
            {error}
          </div>
        )}
        ...
        <div className="animate-fade-slide-up stagger-5 pb-8">
          <button
            ...
            disabled={loading}
            className="w-full btn-primary py-4 text-base relative overflow-hidden disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
